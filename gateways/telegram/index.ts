  ...
  subs.push(
    nc.subscribe("valor.missions.*.active", {
      callback: (_err, msg) => {
        if (_err || !principalChatId) return;
        try {
          const envelope = decode<{ mission_id: string; operative: string }>(msg);
          const p = envelope.payload;
          bot.sendMessage(principalChatId, `\ud83d\udce1 *${p.mission_id}* picked up by *${p.operative}*`, { parse_mode: "Markdown" }).catch(() => {});
        } catch { /* ignore */ }
      },
    }),
  );
    ...
    nc.subscribe("valor.missions.*.complete", {
      callback: (_err, msg) => {
        ...
        try {
          const envelope = decode<NatsSitrep>(msg);
          const p = envelope.payload;
          bot.sendMessage(principalChatId, `\u2705 *${p.mission_id}* \u2014 COMPLETE\n\n${p.summary}`, { parse_mode: "Markdown" }).catch(() => {});
        } catch { /* ignore */ }
      },
    }),
  );
