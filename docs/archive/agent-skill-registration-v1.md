> ⚠️ **SUPERSEDED** — This document is archived. It describes a direct `POST /agents` registration flow that is now Director-locked and will return **403** for agents.
>
> **Use [`SKILL.md`](../../SKILL.md) instead.** The current onboarding flow uses agent cards (`POST /agent-cards`).
> For a quick-start guide, see [`docs/agent-quickstart.md`](../agent-quickstart.md).
